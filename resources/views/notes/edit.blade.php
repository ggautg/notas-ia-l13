...
<body class="bg-gray-950 text-white font-sans min-h-screen relative">
    ...
    <div class="relative max-w-2xl mx-auto px-6 pt-14 pb-20">
        ...
        <form action="/notes/{{ $note->id }}" method="POST" enctype="multipart/form-data"
            x-data="{ loading: false, emoji: '{{ $note->emoji }}' }" @submit="loading = true"
            ...
            <div class="mb-4">
                ...
                <textarea name="content" id="content" rows="5"
                    class="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-indigo-500 transition">{{ $note->content }}</textarea>
            </div>
            ...
            <div class="mb-6">
                @if ($note->image)
                    <img src="{{ Storage::url($note->image) }}" alt="{{ $note->title }}" class="w-32 rounded-lg mb-3">
                @endif
                <label for="image"
                    class="block text-xs font-semibold text-gray-400 mb-1.5 uppercase tracking-wide">Cambiar imagen
                    (opcional)</label>
                <input type="file" name="image" id="image" accept="image/*" class="w-full text-sm text-gray-400">
            </div>

            <button type="submit" :disabled="loading" x-text="loading ? 'Actualizando...' : 'Actualizar'"
